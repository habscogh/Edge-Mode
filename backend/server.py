...
async def calculate_challenge_score(user_id: str, challenge: dict) -> float:
    ...
    # Query sessions within challenge period
    query = {
        'user_id': user_id,
...
async def create_automated_challenges():
    ...
    # Weekly challenges - create on Monday for the current week
    if today.weekday() == 0:  # Monday
        week_start = today
        week_end = today + timedelta(days=6)
        
        # Create pillar-specific weekly challenges
        for pillar in PILLARS[:3]:  # Top 3 pillars for variety
        ...
        # Create general weekly challenges
        general_challenges = [
    ...
    # Monthly challenges - create on the 1st of each month
    if today.day == 1:
        month_start = today
...
async def join_challenge(join_data: ChallengeJoin, current_user: dict = Depends(get_current_user)):
    ...
    # Calculate initial score based on existing sessions
...
@api_router.post("/challenges/leave/{challenge_id}")
...
async def get_challenge_leaderboard(challenge_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # First update all participant scores
...
# Admin endpoint to manually create a challenge
@api_router.post("/challenges/admin/create")
...
# Scheduled job functions for challenges
